...
export default function AssignmentManager() {
  ...
  const saveEdit = (id) => {
    if (currentFolder) {
      const updatedAssignments = currentFolder.assignments.map(assignment => 
        assignment.id === id ? { ...assignment, title: editingTitle } : assignment
      )
      updateCurrentFolder(updatedAssignments)
      setEditingId(null)
    }
      ...
      {currentFolder ? (
        <div>
          <div className="flex items-center mb-6">
            ...
            <button onClick={() => startEditing(currentFolder.id, currentFolder.name)} className="text-blue-500 hover:text-blue-400 p-1 mr-2">
          ...
          {editingId === currentFolder.id ? (
            <div className="flex items-center mb-4">
              <input
                ...
                autoFocus
              />
              <button onClick={() => editFolder(currentFolder.id, editingTitle)} className="text-green-500 hover:text-green-400 p-1 mr-2">
            ...
            {currentFolder.assignments.map((assignment) => (
              <div key={assignment.id} className={`bg-gray-800 rounded-lg p-6 flex justify-between items-center ${assignment.completed ? 'opacity-50' : ''}`}>
                  ...
                  {editingId === assignment.id ? (
                    <div className="flex items-center">
                      <input
                        ...
                        autoFocus
                      />
                      <button onClick={() => saveEdit(assignment.id)} className="text-green-500 hover:text-green-400 p-1" aria-label="Save edit">
                        <IconCheck className="w-5 h-5" />
                      </button>
                ...
                <div className="flex items-center">
                  ...
                  {editingId !== assignment.id && (
                    <button
                      onClick={() => startEditing(assignment.id, assignment.title)}
